...
wxEnhCreate='yes'
# List of wxtoimg enhancements, please read wxtoimg docs
# Commons are: MCIR, MSA, MSA-precip, HVC, HVC-precip, HVCT, HVCT-precip, therm
wxEnhList = [ 'MCIR', 'ZA', 'NO', 'HVC-precip', 'therm' ]
...
# SFPG
	...
	if sfpgLink in ('yes', 'y', '1'):
	    sciezka_plik=imgdir+'/'+satName+'/'+fileNameC+'-MCIR-nomap.jpg'
	    sciezka_plik2=imgdir+'/'+satName+'/_image.jpg'
    	    if os.path.isfile(sciezka_plik2):
		os.unlink(sciezka_plik2)
	    os.symlink(sciezka_plik,sciezka_plik2)
...
def findNextPass():
    predictions = [pypredict.aoslos(s,minElev,stationLat,stationLon,stationAlt,tleFileDir) for s in satellites]
